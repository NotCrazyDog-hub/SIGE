...
@section('page_title', 'Estagiários')

@section('content')

    <div class="flex justify-between items-center mb-6">
        <h1 class="text-xl font-semibold">Lista de estagiários</h1>
        <a href="{{ route('interns.create') }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded shadow">+ Adicionar</a>
    </div>
    ...
    <table class="w-full border">
        <thead>
            <tr class="bg-gray-50 border">
                ...
                <th class="p-3 text-center text-sm font-semibold border-r">Curso</th>
                <th class="p-3 text-center text-sm font-semibold border-r">Telefone</th>
                <th class="p-3 text-center text-sm font-semibold border-r">Status</th>
                ...
                <tr class="border-b hover:bg-gray-100">
                    ...
                    <td class="p-3">
                        <div class="flex items-center gap-3">
                            <a href="{{ route('interns.edit', ['intern' => $intern->id]) }}" class="text-center text-lg">✏</a>

                            <form action="{{ route('interns.destroy', ['intern' => $intern->id]) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <input type="submit" value="🗑" class="text-center text-lg cursor-pointer bg-transparent" onclick="return confirm('Tem certeza que quer deletar este estagiário?')">
                            </form>
                        </div>
                    </td>
                </tr>
