...
@section('page_title', 'Edição de vagas')
...
@section('content')
    ...
    <div class="bg-white border rounded-lg p-6 shadow-sm">
        <form action="{{ route('vacancies.update', ['vacancy' => $vacancy->id]) }}" method="post" class="space-y-5">
            @csrf
            @method('PUT')
            <div>
                <label class="block mb-1 font-medium">Título da vaga:</label>
                <input type="text" name="title" placeholder="Insira o título da vaga" value="{{ old('title', $vacancy->title) }}" class="w-full border rounded px-3 py-2">
                @error('title')
            ...
            <div>
                ...
                <textarea name="description" placeholder="Insira a descrição da vaga aqui"
                          class="w-full border rounded px-3 py-2 h-28">{{ old('description', $vacancy->description) }}</textarea>
                @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" name="workload" placeholder="Insira a carga horária semanal aqui" value="{{ old('workload', $vacancy->workload) }}" class="w-full border rounded px-3 py-2">
                @error('workload')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
